...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($customer_name) || empty($email) || empty($product_name) || !isset($quantity) || !isset($price)) {
        die('Error: Customer name, email, product id, quantity and price are mandatory.');
    }
    ...
    try {
        ...
        $sql = "INSERT INTO orders (customer_name, customer_email, product_id, quantity, price) 
                VALUES (:customer_name, :customer_email, :product_id, :quantity, :price)";
